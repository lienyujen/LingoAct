...
LingoAct stands for Intelligent Teaching, Engagement, Response and Classroom Technology.

LingoAct 是提供教師、講師、訓練師與演講者使用的即時課堂互動系統。講師在 Windows 端建立場次，學員掃描 QR Code 後即可用手機瀏覽器加入，不需要安裝 App。
...
1. 從 [Releases](https://github.com/lienyujen/LingoAct/releases) 下載 `LingoAct.zip`，解壓後執行裡面的 `LingoAct.exe`。
2. 在 [Supabase](https://supabase.com/dashboard) 免費建立一個專案。
3. 開啟 LingoAct，出現設定畫面時填入專案識別碼與 publishable key。
4. 展開「還沒建立後端？讓 LingoAct 幫你部署」，貼上一組
   [Supabase 存取權杖](https://supabase.com/dashboard/account/tokens)與 Gemini API key，按下自動部署。

LingoAct 會替你建立資料表、部署 Edge Functions 並設定金鑰，不需要安裝 Node 或 Supabase CLI。
...
5. Windows：把自己的公開設定打包進 `LingoAct.exe`。

**學員端網頁不需要自行部署。** 加入連結會帶著你的 Supabase 專案識別碼，共用的學員端會據此連到你的專案，課堂資料不會混在一起。

完整繁體中文教學請見 [`docs/LingoAct-從零部署與打包教學.md`](docs/LingoAct-從零部署與打包教學.md)。可安裝 [`lingoact-self-deploy`](skills/lingoact-self-deploy/SKILL.md) skill，讓 Codex 依序引導部署：
...
學員端固定顯示 LingoAct 作者的 [Facebook](https://www.facebook.com/lienyujen) 與 [YouTube](https://www.youtube.com/@lienlaoshi) 連結。
