...
const APP_PROFILES = {
  ...
  ncacls: { appId: 'org.ncacls.lingoact.desktop', displayName: 'LingoAct NCACLS' },
}
...
function createWindow() {
  ...
  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedUrl) => {
    console.error('LingoAct failed to load', { errorCode, errorDescription, validatedUrl })
  })

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('LingoAct renderer process gone', details)
  })
...
ipcMain.handle('capture:start-selection', async () => {
  if (!mainWindow || mainWindow.isDestroyed()) throw new Error('LingoAct presenter window is unavailable.')
