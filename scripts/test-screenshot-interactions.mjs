...
try {
  ...
  const works = await presenter({ action: 'teaching_samples', questionId: question.id })
  assert.equal(works.samples.length, 2)
  assert.ok(works.samples.every(work => work.text.includes('起床')))
