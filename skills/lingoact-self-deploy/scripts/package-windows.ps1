...
$output = Join-Path $env:TEMP ("LingoAct-package-{0}" -f [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
$envContent = @"
...
try {
  ...
  $source = Join-Path $output 'LingoAct.exe'
  if (-not (Test-Path -LiteralPath $source)) { throw 'electron-builder did not produce LingoAct.exe.' }
  Copy-Item -LiteralPath $source -Destination (Join-Path $root 'LingoAct.exe') -Force
  $result = Get-Item -LiteralPath (Join-Path $root 'LingoAct.exe')
