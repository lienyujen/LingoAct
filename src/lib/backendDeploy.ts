...
export const canDeployBackend = typeof window !== 'undefined' && Boolean(window.lingoActDesktop?.supabaseManagement)
...
async function call(request: ManagementRequest) {
  const bridge = window.lingoActDesktop?.supabaseManagement
  if (!bridge) throw new Error('自動部署只能在 LingoAct 桌面版使用。')
...
export async function setOwnerKey(ref: string, token: string, ownerKey: string) {
  const result = await call({
    ...
    method: 'POST',
    token,
    json: [{ name: 'LINGOACT_OWNER_KEY', value: ownerKey }],
  })
