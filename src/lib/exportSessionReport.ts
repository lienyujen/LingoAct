...
export async function exportSessionReport(data: SessionReportData, analysis: SessionAnalysis, metrics: SessionMetrics) {
  ...
  const reportTitle = overview.addRow([`LingoAct 課堂互動報告｜${data.session.title}`])
  ...
  anchor.href = url
  anchor.download = `LingoAct-${safeTitle}-${new Date().toISOString().slice(0, 10)}.xlsx`
